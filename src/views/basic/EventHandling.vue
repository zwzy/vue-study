    ...
    <a-flex gap="middle" wrap="wrap">
      <a-card title="1、event normal">
        <div @click="say('wrap')" class="wrapper">
          <div @click="say('inner')" class="inner">
            内部
          </div>
        </div>
      </a-card>
      <a-card title="2、event inner stop 防止冒泡触发父级事件">
        <div @click="say('wrap')" class="wrapper">
          <div @click.stop="say('inner')" class="inner">
            内部
          </div>
        </div>
      </a-card>
      ...
      <a-card title="4、event inner once 只触发一次">
        <div @click="say('wrap')" class="wrapper">
          <div @click.once="say('inner')" class="inner">
            内部
          </div>
        </div>
      </a-card>
      <a-card title="5、event wrap capture 打乱冒泡顺序，先执行有capture的事件，都有从外到内事件执行">
        <div @click.capture="say('wrap')" class="wrapper">
          <div @click.capture="say('inner')" class="inner">
            内部
          </div>
        </div>
      </a-card>
      ...
      <a-card title="9、event inner click middle 鼠标事件 left right middle">
        <div class="wrapper" @click="say('wrap')">
          <div @click.middle="say('inner middle')" class="inner">
            enter
          </div>
        </div>
      </a-card>
